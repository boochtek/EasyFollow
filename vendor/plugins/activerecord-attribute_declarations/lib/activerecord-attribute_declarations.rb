...
module ActiveRecord::AttributeDeclarations
  ...
  private
    ...
    def add_validations_from_attribute_declaration(name)
      ...
      validates_length_of name, :minimum => decl[:min_length], :allow_nil => true   if decl[:min_length]
      validates_length_of name, :maximum => decl[:max_length], :allow_nil => true   if decl[:max_length]
      validates_format_of name, :with => decl[:format], :allow_nil => true          if decl[:format]
